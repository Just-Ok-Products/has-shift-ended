...
export class MealVoucherService {
  ...
  public getDetail(intervals: Date[]): MealVoucherDetail {
    ...
    const lunch = lunchGapIndex !== -1;

    const morningIntervals = lunch ? workIntervals.slice(0, lunchGapIndex + 1) : workIntervals;
    const morning = this.sumMinutes(morningIntervals) >= this.MORNING_MIN_MINUTES;

    const afternoon = lunch && this.sumMinutes(workIntervals.slice(lunchGapIndex + 1)) >= this.AFTERNOON_MIN_MINUTES;
